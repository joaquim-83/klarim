...
def test_tier_split_counts():
    assert len(scanner.ALL_CHECKS) == 29
...
def test_summary_payload_free_locks_paid_and_hides_detail():
    ...
    assert len(p["free_checks"]) == 15 and len(p["paid_checks"]) == 14
