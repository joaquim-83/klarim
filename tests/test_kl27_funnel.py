...
def test_tier_split_counts():
    # 40 checks após o KL-36 (DNS 37-40); 15 gratuitos, 25 pagos (ORDER>15).
    assert len(scanner.ALL_CHECKS) == 40
...
def test_summary_payload_free_locks_paid_and_hides_detail():
    ...
    assert len(p["free_checks"]) == 15 and len(p["paid_checks"]) == 25
