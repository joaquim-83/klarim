...
def test_tier_split_counts():
    # 36 checks após o KL-32 (headers 31-36); 15 gratuitos, 21 pagos (ORDER>15).
    assert len(scanner.ALL_CHECKS) == 36
...
def test_summary_payload_free_locks_paid_and_hides_detail():
    ...
    assert len(p["free_checks"]) == 15 and len(p["paid_checks"]) == 21
