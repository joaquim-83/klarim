...
- **Tech detect:** do mesmo response bruto, `scanner/tech_detector.py::detect_tech_stack(headers, html, dns, ssl, domain=None)` (pura) extrai tecnografia (~50 scripts, headers, meta, DNS TXT, SSL) + `site_type` + `site_status`. `persist_tech_detection` grava (resiliente) em `site_tech_stack` (batch idempotente, **por scan_id** → leitura usa o scan mais recente, então re-scan reclassifica sozinho), `targets.email_provider`/`related_domains`/`site_type`, `site_status_log`, `company_name` só-se-vazio. Público = `GET /public/tech-summary/{domain}`.
- **Fingerprint de plataforma = SAME-ORIGIN + múltiplos sinais (KL-165):** evidência de plataforma (WordPress/Shopify/…) baseada em URL só conta se a URL for **do próprio domínio** (`is_same_origin`, comparação por eTLD+1; relativas contam) — refs cross-origin (CDN/logos/embeds) são ignoradas. **WordPress exige 2+ sinais** same-origin (`_detect_platforms`/`_PLATFORM_SPECS`): generator/cookie `wp_settings` são **fortes** (bastam sozinhos); `/wp-content/`,`/wp-includes/`,`wp-emoji`,`/wp-json` (URL same-origin) + `wp-block-` (markup) são **fracos** (≥2). O detector é PURO (não sonda `/wp-admin`/`/wp-json` — honeypot 200 não influi). `cdn.shopify.com` saiu do fingerprint (era embed-prone) → Shopify vem de header `x-shopify-stage`/cookie `_shopify_*` (same-origin). O worker passa `domain=domain_of(url)`; o backfill usa o `domain` do payload GCS; o tool `/api/tools/tech` passa o host.
