...
def extract_san_domains(entry: dict) -> List[str]:
    """Extrai os domínios (SAN + CN) de uma entrada de get-entries.
    ...
    precert (1) o cert completo vem no início do `extra_data`.
    """
    try:
...
class CTLogPoller:
    ...
    def _ingest(self, entry: dict) -> None:
        """Processa uma entrada de CT: extrai SANs, filtra .com.br, joga no buffer."""
        self.total_seen += 1
        for raw in extract_san_domains(entry):
