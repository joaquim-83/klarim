# Klarim — Histórico de entregas (HISTORY)

> **Este arquivo é o registro histórico completo.** Ele preserva, na íntegra e sem
> perda, o conteúdo do antigo `claude.md` (que cresceu para ~164k chars / 50 seções
> em 10 dias de implementação). Foi arquivado aqui na reestruturação de **2026-07-16**
> (ver `claude/reports/reestruturacao_docs.md`).
>
> **Onde procurar agora:**
> - **Guia do agente CLI (regras + estado atual):** `claude.md` (enxuto, na raiz)
> - **Arquitetura detalhada:** `docs/ARCHITECTURE.md`
> - **Endpoints da API + tools MCP:** `docs/API.md`
> - **Deploy e variáveis de ambiente:** `docs/DEPLOY.md`
> - **Políticas de segurança:** `docs/SECURITY.md`
> - **Histórico de cada tarefa (KL-xxx):** `claude/reports/`
>
> Cada seção abaixo corresponde a uma entrega (card KL-xxx) e está **intacta** como
> foi escrita. Use como referência de "por que" e "como" cada peça foi construída.
...
| 6 | Como rodar | — |
| 7 | Fluxo de trabalho de uma tarefa | — |
| 8 | Deploy e CI/CD (GCP) | KL-3 |
| 9 | Relatórios PDF (`reporter/`) | — |
| 10 | Interface web (`frontend/`) + HTTPS + hardening | KL-6 |
| 11 | Pagamento — AbacatePay PIX | KL-7 |
| 12 | E-mail — Resend | KL-8 |
| 13 | Cache de scan + feedback de e-mail | KL-9 |
| 14 | Recuperação de relatórios | KL-10 |
| 15 | Discovery Worker | KL-11 / KL-15 |
| 16 | Alert Worker + calibração do semáforo | KL-12 |
| 17 | Re-scan Worker + e-mail de evolução | KL-13 |
| 18 | Dashboard admin | KL-14 |
| 19 | Integração completa (scans públicos + rastreabilidade) | KL-17 |
| 20 | Dashboard operacional | KL-16 |
| 21 | Mensagens de risco dinâmicas | KL-20 |
| 22 | Tracking da jornada do lead | KL-21 |
| 23 | Controle de bounce | KL-24 |
| 24 | Servidor MCP | KL-18 |
| 25 | Scan público verificado por e-mail | KL-25 |
| 26 | Funil de conversão (free 15 / R$ 19) | KL-27 |
| 27 | Sites Monitorados (selo score 100) | KL-29 |
...
| 32 | Componentes vulneráveis + CVE matching (check_30) | KL-33 |
| 33 | Análise de qualidade de headers + headers modernos | KL-32 |
| 34 | DNS security expandido (checks 37–40) | KL-36 |
| 35 | TLS profundo (checks 41–44) | KL-37 |
| 36 | Content analysis passivo (checks 45–48) | KL-38 |
| 37 | Enriquecimento por IA | KL-47A / KL-50 L5 |
| 38 | Taxonomia de setores (48 setores) | KL-54 |
| 39 | Plataforma pública em Astro (+ fases 2–5) | KL-51 |
| 40 | Classificação CNAE multi-setor | KL-55 |
| 41 | Gestão de landing + inbox de e-mail | KL-56 |
| 42 | Perfil no resultado + gestão de conta | KL-57 |
| 43 | Score social: widget + card + ranking + selo | KL-42 |
| 44 | Cobertura MCP + fix `last_scan_at` | — |
| 45 | Gestão de Leads (scan_leads + PQL) | KL-61 |
| 46 | Rastreabilidade unificada de e-mails | KL-62 |
| 47 | Vigílias core — monitoramento contínuo | KL-44 P2 |
| 48 | MCP OAuth 2.1 + PKCE | KL-63 |
| 49 | Configurações editáveis ao vivo + senha + rotação MCP | KL-44 |
| 50 | SEO + sinais de autoridade | KL-65 |

---

## Conteúdo original preservado (antigo `claude.md`)

> **Nota histórica:** o preâmbulo original dizia "Leia este arquivo antes de tocar no
> código. Ele é o onboarding obrigatório para qualquer agente Claude". Esse papel agora
> é do `claude.md` enxuto na raiz. O conteúdo abaixo segue **exatamente** como estava.
...
Monitoramento silencioso para contas **Pro/Agency**: 5 vigílias (SSL, domínio, score,
e-mail, reputação) rodam a cada 6h e só avisam o dono quando algo importa. **100% passivo**
e **não altera o score de segurança**.

- **Tabelas** (`discovery/store.py`): `vigilias` (1 por `user_id`+`site_domain`+`tipo`;
  `last_status`/`last_data` JSONB guarda o estado anti-spam) e `vigilia_alerts` (histórico).
- **Lógica** (`api/vigilias.py`, módulo puro): cada `check_*` lê o último scan (SSL=check_03
  `days_left`; score=2 scans; e-mail=checks 21-23 PASS→FAIL; reputação=checks 28-29 FAIL) ou
  faz **RDAP** (domínio, `.br`→registro.br, cache 24h). Anti-spam: SSL/domínio por
  **threshold** (`alerted_thresholds`, renovação limpa), score/e-mail por `last_alerted_scan_id`,
  reputação por conjunto de blacklists. `run_vigilia_check` é o dispatcher (nunca levanta).
- **Worker** (`discovery/vigilia_worker.py`, no container discovery): por ciclo pega as
  vigílias vencidas, faz **enforcement de plano** (`plans.get_subscription`, expiração lazy do
  trial; plano não permite → **desativa**; erro transiente → pula), roda o check (timeout 30s),
  cria alerta + envia e-mail (proativo → respeita blocklist), reagenda. **`worker_control`**
  ganhou `"vigilia"` (`cycle_hours`/`max_per_cycle`) — `pause_worker`/`resume_worker` valem.
  **Começa pausado** (o seed grava a pausa; o dono ativa via `resume_worker vigilia`).
- **Auto-criação:** `account_add_site` cria as vigílias do plano (fire-and-forget); a mudança
  de plano (`PATCH /admin/subscriptions/{id}/plan` + bulk) chama `_sync_user_vigilias`
  (upgrade cria, downgrade desativa — nunca deleta).
- **E-mail** (`notifier/templates/vigilia_*.html` + `send_vigilia_alert`): dark-mode, bloco
  "Texto para o técnico", CTA → `/site/{domínio}`. `email` usa `vigilia_email_security.html`.
- **API:** admin `GET /admin/vigilias[/stats|/{id}]` + `/admin/vigilia-alerts` (JWT admin);
  usuário `GET /account/vigilias` + `/account/vigilia-alerts` (JWT de usuário, **filtrado por
  `user_id` — IDOR-safe**, rate limit 10/min). MCP: `get_vigilia_stats`, `list_vigilia_alerts`.
- **Painel:** `/painel/vigilias` (Astro `VigiliaPage.jsx`). **Seed:** `scripts/seed_vigilias.py`.

**Regra inviolável:** vigília é **passiva** e não muda o score; o worker **começa pausado**
(só o dono ativa via MCP); enforcement de plano é servidor-autoritativo e **nunca** desativa
por erro transiente de lookup; e-mail de vigília **respeita a blocklist** (proativo, KL-24/62).

## 48. MCP OAuth 2.1 + PKCE (KL-63)

O MCP aceita **OAuth 2.1 + PKCE** (spec MCP nov/2025) além do token estático. O operador
adiciona a URL limpa `https://klarim.net/mcp/sse` no Claude → login com a senha admin numa
página do Klarim → access token JWT (1h) + refresh (30d, rotação). O Klarim é o **próprio
authorization server** (single-tenant). **`MCP_API_KEY` continua como fallback** (CLI +
conexões existentes) — nada removido.

- **`mcp_server/oauth.py`** — metadata (RFC 9728/8414), DCR (RFC 7591), authorize (login +
  code), token (authcode+PKCE S256 / refresh+rotação), JWT mint/validate. Reusa o Redis da
  API (`api.main._cache`) + `_redis_allow`. Sem Redis → OAuth degrada 503 (estático segue).
- **Roteamento:** `/.well-known/oauth-protected-resource` + `/.well-known/oauth-authorization-server`
  são rotas **públicas** da API (raiz); o Nginx ganhou `location ^~ /.well-known/oauth-`
  (prioridade sobre `~ /\.`, proxy identidade p/ `api:8000`). `/mcp/authorize|token|register`
  são rotas Starlette no `mcp_app` (sob o `/mcp/` do Nginx), **isentas** na `MCPAuthMiddleware`.
- **`MCPAuthMiddleware` (auth.py)** aceita: (1) Bearer JWT OAuth (`validate_access_token`:
  assinatura/iss/aud/exp/typ/scope), (2) `MCP_API_KEY` estático (Bearer, constant-time), (3)
  `?token=` (JWT propagado ao `/messages/` **ou** `MCP_API_KEY`). Fail-closed. 401 traz
  `WWW-Authenticate: Bearer resource_metadata="…"`.
- **Segurança:** PKCE S256 obrigatório, `redirect_uri` casada com a registrada (só HTTPS ou
  HTTP loopback — anti open redirect), code one-time 60s, refresh rotacionado, rate limit
  register 5/h + authorize 5/min, senha admin constant-time, HTML escapado (anti-XSS).
- **Config (`.env` VM):** opcional `MCP_JWT_SECRET` (senão `JWT_SECRET`), `MCP_ISSUER`
  (default `https://klarim.net`); reusa `ADMIN_PASSWORD` e `MCP_API_KEY`. **Redis keys:**
  `mcp:client:{id}` (30d), `mcp:auth_code:{code}` (60s), `mcp:refresh:{token}` (30d).

**Regra inviolável:** o token estático permanece como fallback; o fluxo OAuth é PKCE-only
(cliente público, sem secret); nenhuma rota do fluxo (authorize/token/register) exige o
token que ela mesma emite; `redirect_uri` sempre validada contra a registrada.

## 49. Configurações editáveis ao vivo + senha + rotação MCP (KL-44)

A página `/painel/config` virou editável: 10 parâmetros operacionais ajustados sem
redeploy, troca de senha do admin e rotação do token MCP.

- **Tabela `admin_settings`** (`key`/`value`/`updated_by`/`updated_at`) + `store.get_setting(
  key, default)` = **banco → `.env` → default** (fail-open). Guarda também
  `ADMIN_PASSWORD_HASH` (bcrypt) e `MCP_API_KEY` (rotação).
- **Workers releem por ciclo:** alert/rescan/discovery têm um reload no `run_cycle`; o scan
  relê `WORKER_MAX_SCANS_PER_HOUR` por iteração — via `get_setting`. Mudança no painel vale
  no próximo ciclo (antes era só no `__init__` → exigia redeploy).
- **`verify_admin_password`** (`api/main.py`): hash bcrypt do banco (prioridade) →
  `ADMIN_PASSWORD` do `.env` (legado/1º acesso). Usado por `/auth/login` **e**
  `/mcp/authorize` (KL-63). Primeiro acesso = env; após trocar no painel = hash do banco.
- **Endpoints (`/admin`, JWT):** `GET /admin/config` (params + token mascarado, nunca o
  hash/token inteiro), `PUT /admin/config/{key}` (whitelist `_CONFIG_PARAMS` + faixa int,
  10/min), `POST /admin/config/reset/{key}`, `PATCH /admin/password` (senha atual + força
  ≥12 c/ maiúscula+minúscula+número, 3/min, invalida refresh OAuth), `POST
  /admin/rotate-mcp-token` (CSPRNG, aplica em `os.environ` runtime + banco, 1/h, invalida
  refresh, token mostrado 1×), `GET /admin/system-info` (versão/uptime/redis).
- **`_load_runtime_overrides()`** no boot carrega o `MCP_API_KEY` do banco → `os.environ`
  (a rotação sobrevive a restart; o middleware lê `os.environ`, hot path).
- **Frontend `ConfigPage.jsx`:** tabela editável (✏️ inline, badge db/env, Resetar) +
  seção Segurança (senha + rotação do token via modal) + Informações. `adminApi.js`:
  `configList`/`configPut`/`configReset`/`changePassword`/`rotateMcpToken`/`systemInfo`.

**Regra inviolável:** banco > `.env`; a senha em texto puro do `.env` é só fallback;
`get_setting` é fail-open (erro de banco nunca pausa worker); nenhuma resposta expõe
senha/hash/token inteiro; trocar senha ou rotacionar token invalida os refresh tokens OAuth.

## 50. SEO + sinais de autoridade (KL-65)

O AI Overview do Google alucinava que `klarim.net` era phishing (confundia "Klarim" com
"Clarim"). Correção **só de frontend/Nginx** (sem backend):
- **Schema.org (JSON-LD)** no `web/src/layouts/Base.astro` (público; o admin usa
  `AdminLayout`, sem isto): **Organization + WebSite** em toda página pública;
  **SoftwareApplication** na landing (`index.astro`); **WebPage + Review** em cada perfil
  `/site/{domain}` (o Klarim é o `author` da Review). O prop `jsonLd` do `Base` aceita
  objeto ou array. JSON-LD é `type=ld+json` (dado) → **não** entra na CSP `script-src`.
- **security.txt (RFC 9116):** `frontend/nginx/security.txt` → `COPY` no Dockerfile →
  `/etc/nginx/klarim_security.txt`, servido por `location = /.well-known/security.txt {
  alias …; }` nos 3 blocos (http.conf + 2× https.conf.template). `location =` vence o
  `~ /\.`. Mesmo padrão do `mta-sts.txt`.
- **humans.txt + og-image.png** em `frontend/public/` (servidos em `/humans.txt` e
  `/og-image.png` pelo `location /` do container web). O `og-image.png` (1200×630) foi
  gerado com Pillow. O `Base` usa `og-image.png` como `og:image` default + `twitter:card
  summary_large_image` + `<meta name=author>`.
- **robots.txt** (`web/public/robots.txt`, servido pelo Astro): `Disallow /painel/`,
  `/api/admin/`, `/mcp/` + sitemap.

**Regra inviolável:** não remover o JSON-LD (Organization/WebSite/SoftwareApplication/
Review) — é o sinal anti-alucinação para o Google. `og-image.png`/`humans.txt` ficam em
`frontend/public/`; a `security.txt` é servida via alias Nginx (o `/.well-known/` é
bloqueado pelo `~ /\.` — precisa do `location =`).
