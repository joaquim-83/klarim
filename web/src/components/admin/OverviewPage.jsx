...
// Portado de frontend/src/pages/admin/Overview.jsx (KL-51 fase 1). Recharts numa ilha
// client:load (não afeta o bundle público). <Link> → <a href> (nav dura). Os 4 gráficos
// (donut status, bar plataforma, 2 line charts diários) são preservados.
const TOOLTIP_STYLE = {
...
export default function OverviewPage() {
  const { data, loading, error } = useAsync(() =>
    Promise.all([
      ...
      admin.scans({ limit: 10, distinct_url: true }),
      admin.dashboardStats(),
      admin.systemStatus().catch(() => null),
        ...
        {data.system && (
          <Card title="Saúde do sistema">
            ...
            <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 text-sm">
              ...
              <a href="/painel/sistema" className="text-klarim-alert hover:underline">ver detalhes →</a>
            </div>
          </Card>
        )}
                  ...
                  {data.recent.map((s) => (
                    <tr key={s.id} className="border-t border-klarim-border">
                      ...
                      <td className="py-2 text-right">
                        <a href={`/painel/scans/${s.id}`} className="text-klarim-alert hover:underline">detalhes</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
  ...
  return <AdminShell active="overview">{body}</AdminShell>
}
