...
import { CLASS_META, ClassBadge } from './LeadShared'
import AdminShell from './AdminShell'

// Portado de frontend/src/pages/admin/LeadDetalhe.jsx (KL-51 fase 2). useParams → pathname;
// Link → <a href>; CLASS_META/ClassBadge de LeadShared (não de Leads).
...
export default function LeadDetalhePage() {
  ...
  else if (!lead) body = <ErrorBox message="Lead não encontrado." />
  else {
    ...
    body = (
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <a href="/painel/leads" className="text-xs text-klarim-muted hover:underline">← Leads</a>
                ...
                {(lead.scans || []).map((s) => (
                  <tr key={s.id} className="border-t border-klarim-border">
                    <td className="py-2 pr-3 font-mono text-xs text-klarim-muted">
                      <a href={`/painel/scans/${s.id}`} className="hover:underline">
                        {s.domain || s.url}
                      </a>
                    </td>
  ...
  return <AdminShell active="leads">{body}</AdminShell>
}
