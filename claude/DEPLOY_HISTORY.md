...
| 2026-08-18 | KL-169 (priorização de e-mail: pessoais primeiro, genéricos fallback) | run #32124523532 (commit `be52e6a`) | Test·Nginx·Astro·Deploy·Security Gate 5/5 verdes. **Solução definitiva** das 3 tentativas (KL-146 só-score / KL-167 filtrou-parou / KL-168 desligou-25%bounce): **priorizar na SELEÇÃO** — `get_eligible_targets_for_alert` ordena pessoais (0) antes de genéricos (1) via `GENERIC_PREFIX_SQL_REGEX`, depois `last_scan_score ASC`. Como o ciclo busca só 200 de milhares, o ORDER BY decide quem entra → genéricos são fallback, NUNCA bloqueados. Removidos `ALERT_SKIP_GENERIC`/`is_generic_alert_email`/`blocked_generic`; `is_generic_email` vira classificador (7 prefixos); stats ganham `sent_personal`/`sent_generic`. **Pós-deploy (VM):** código no ar (7 prefixos, sem `skip_generic`); **ORDER BY live — primeiros 20 elegíveis = 20/20 PESSOAIS** (worst-score-first); **envios >10:06 UTC = 3/3 pessoais, 0 genéricos**; hard bounce 4,8% (<9%); `admin_settings.ALERT_SKIP_GENERIC` (estava `'true'`) **removido** |
